 ...
 * 하단 탭바.
 *
 * 모바일에서는 이것이 유일한 주 메뉴다. 헤더의 메뉴트리는 같은 목적지를
 * 두 벌로 만들 뿐이라 좁은 폭에서 접었다 — 그래서 GNB의 항목이 여기 전부
 * 들어와야 한다. 공지를 뺄 수 없는 이유도 그것이다(예전엔 방문 빈도를
 * 근거로 뺐는데, 헤더에 남아 있을 때 이야기다).
 *
 * - 내 정보는 GNB에 없지만 반복 진입점이고, 앱형 탭바에서 마지막 칸은
 *   관례적으로 내 정보 자리다.
 * - 홈을 넣은 이유는 모바일에서 홈으로 돌아갈 길이 헤더 로고 하나뿐이라서다.
 *
 * 여섯 칸은 320px에서 칸당 49px이다. 아이콘 20px에 라벨 두 글자가 들어가는
 * 하한이라 더 늘릴 수는 없다.
 */
