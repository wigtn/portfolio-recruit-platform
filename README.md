...
| AI | 글 상세의 참고 답변 생성, 화면을 직접 조작하는 데모 도우미 |

데모 도우미는 말만 하지 않습니다. 화면을 옮기고, 역할을 바꾸고, 커서로 짚고,
...
아무것도 넣지 않아도 데모는 완결됩니다. AI는 준비된 답변으로 동작하고,
데모 문의사항은 외부로 전송하지 않고 브라우저에만 저장합니다.

| 변수 | 없으면 |
|---|---|
| `WIGTN_OPENAI_API_KEY` | AI 답변과 데모 도우미가 준비된 답변으로 동작 |
